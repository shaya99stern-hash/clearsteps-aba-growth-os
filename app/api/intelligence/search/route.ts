...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return NextResponse.json({ ok: false, error: "Enter a valid research request and location." }, { status: 400 });
  ...
  const rows: Array<{ lane: (typeof plan.queries)[number]["lane"]; hit: Awaited<ReturnType<typeof searchPublicWeb>>[number]; enrichment?: Awaited<ReturnType<typeof enrichPublicWebsite>> }> = [];
  const errors: string[] = [];
  ...
  for (let index = 0; index < searchQueries.length; index += 3) {
    ...
    const results = await Promise.all(batch.map(async (planQuery) => {
      try { return { planQuery, hits: await searchPublicWeb(planQuery.query, 5), error: null as string | null }; }
      catch (error) { return { planQuery, hits: [], error: error instanceof Error ? error.message : "search failed" }; }
    }));
  ...
  const uniqueForEnrichment = Array.from(new Map(rows.map((row) => [safeDomain(row.hit.url) ?? row.hit.url, row])).values()).slice(0, 6);
  await Promise.all(uniqueForEnrichment.map(async (row) => { row.enrichment = await enrichPublicWebsite(row.hit.url); }));
  const enrichmentByDomain = new Map(uniqueForEnrichment.map((row) => [safeDomain(row.hit.url), row.enrichment]));
  const resolved = resolveSearchHits(rows.map((row) => ({ ...row, enrichment: row.enrichment ?? enrichmentByDomain.get(safeDomain(row.hit.url)) ?? null })), location).slice(0, maxResults);
  ...
  return NextResponse.json({
    ...
    browser: { source: "Playwright Public Browser", status: browserReady ? "complete" : "unavailable", detail: browserReady ? "runtime available" : "optional runtime not installed; fetch collectors used" },
    screened: rows.length, leads: resolved,
    territory: { location: location || "Unspecified territory", signals, ...territory }, errors,
  });
}
