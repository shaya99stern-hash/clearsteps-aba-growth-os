...
- Browser acquisition implemented: Playwright `1.62.1` is installed as an optional runtime; public-page fallback blocks private-network requests, revalidates redirects, and only runs on weak fetch enrichment. Real chromium-headless-shell launch passed GitHub Actions run `33150961118`.
...
- Tasks phase: task domain rules, `/api/tasks`, PostgreSQL repository, local-first store, CRM linking, due date/priority composer, Open → In Progress → Done board, and durable merge path are implemented. Task acceptance rules pass; final combined lint/build verification is running after exact lint repairs.
- Outreach phase: referral-outreach eligibility rules, suppression enforcement, reviewed recipient selection, editable templates, token personalization preview, explicit no-PHI/manual-review acknowledgment, and saved reviewed campaign drafts are implemented. Sending remains intentionally disabled; combined acceptance/lint/build verification is running.
- CI: GitHub Actions verifies clean install, intelligence rules, task rules, outreach rules, Prisma validate/generate, changed-source ESLint, and full Next.js build. A separate browser workflow installs chromium-headless-shell and launches it for executable verification.
